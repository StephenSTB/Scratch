...
class Main extends Component{
    ...
    getTokens = async () =>{
        if(this.props.web3 !== null){
            ...
            if(tokens !== this.state.scratchTokens){
    ...
    buyCard = async () =>{
        if(this.props.web3 !== null){
            ...
            var cardPrice = await cardRound.methods.getCardPrice().call();

            var fee = 10 ** 17;

            cardPrice = parseInt(cardPrice, 10) + fee + 1000;

            console.log("card price: " + cardPrice);

            await link.methods.approve(scratch.options.address, cardPrice.toString()).send({from: this.props.accounts[0]});
            ...
            var transaction = await scratch.methods.buyScatchCard(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER).toString()).send({from: this.props.accounts[0]});
